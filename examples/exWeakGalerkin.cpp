...
int main(int argc, char *argv[])
{
   ...
   unsigned int test_order  = order + 1;

   FiniteElementCollection *x0_fec, *xb_fec, *test_fec;

   x0_fec = new L2_FECollection(trial_order, dim);
   xb_fec = new RT_Trace_FECollection(trace_order, dim);
   test_fec = new L2_FECollection(test_order, dim);

   FiniteElementSpace *x0_space = new FiniteElementSpace(mesh, x0_fec);
   FiniteElementSpace *xb_space = new FiniteElementSpace(mesh, xb_fec);
   ...
   //    and rhs.
   enum {x0_var, xb_var, NVAR};

   int s0 = x0_space->GetVSize();
   int s1 = xb_space->GetVSize();
